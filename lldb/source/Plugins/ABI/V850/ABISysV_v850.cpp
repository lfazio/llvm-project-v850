...
ABISP
ABISysV_v850::CreateInstance(lldb::ProcessSP process_sp,
                             const ArchSpec &arch) {
...
bool ABISysV_v850::GetArgumentValues(Thread &thread,
  ...
  for (uint32_t value_idx = 0; value_idx < num_values; ++value_idx) {
    ...
    std::optional<uint64_t> opt_byte_size = compiler_type.GetByteSize(&thread);
    if (!opt_byte_size)
      return false;
